			...
			<sec:authorize access="isAuthenticated()">
				<sec:authorize access="not hasRole('ADMIN')">
					<a href="${pageContext.request.contextPath}/order/list"
						style="text-decoration: none; color: #d9534f; font-weight: bold;">📦
						주문내역</a>
				</sec:authorize>
				...
				<sec:authorize access="hasRole('ADMIN')">
					<div style="position: relative; display: inline-block;">
						...
						<div class="admin-dropdown"
							style="display: none; position: absolute; top: 100%; right: 0; background: white; border: 1px solid #eee; border-radius: 8px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); z-index: 1000; padding: 10px; width: 120px;">
							<a href="${pageContext.request.contextPath}/admin/book/list"
								...
								주문관리</a> <a
								href="${pageContext.request.contextPath}/admin/stat/sales"
...
document.addEventListener("DOMContentLoaded", function() {
    const adminTrigger = document.querySelector('.admin-dropdown').previousElementSibling; // '관리자 메뉴' 링크
    const dropdown = document.querySelector('.admin-dropdown');

    // 마우스를 올렸을 때
    adminTrigger.parentElement.addEventListener('mouseenter', () => {
        dropdown.style.display = 'block';
    });

    // 마우스를 뗐을 때
    adminTrigger.parentElement.addEventListener('mouseleave', () => {

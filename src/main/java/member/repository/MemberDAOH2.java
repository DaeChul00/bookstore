...
@Repository // 이 클래스가 데이터 접근(DAO)임을 Spring에 알림
public class MemberDAOH2 implements MemberDAO {
    
    // DB 연결 및 실행을 도와주는 Spring 객체 주입 (이것 하나면 충분합니다!)
    @Autowired
    private JdbcTemplate jdbcTemplate; 
    
    // ❌ 에러의 원인이었던 @Autowired Connection conn; 은 삭제했습니다!
    ...
    public void updateMember(MemberVO vo) {
        // JdbcTemplate을 사용하여 코드가 아주 간결해집니다.
    ...
    public List<MemberVO> findAll(String sort) {
        ...
        try {
            // query 메서드와 BeanPropertyRowMapper를 사용하면 while(rs.next()) 루프 없이 바로 리스트를 반환합니다.
    ...
    public MemberVO selectById(String memberId) {
        // findById와 동일한 로직을 수행하도록 수정해 두었습니다.
